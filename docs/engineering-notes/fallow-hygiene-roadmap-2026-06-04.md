...
- Task detail run-summary helper extraction
  - Source signal: after the circular-dependency pool was cleared, CodeGraph-led triage showed `getTaskDetail` as a high-complexity service hotspot with a moderate 24-symbol impact surface and direct service tests.
  - Completed by extracting the duplicated `latestRun`/`activeRun` summary mapping into a typed local `summarizeTaskRun` helper in `src/lib/orchestration/service/task-detail.ts`.
  - Validation: CodeGraph `status`, `query`, `callers`, `callees`, and `impact` for `getTaskDetail`; focused task-detail, foundation workflow, voice outcome persistence, execution cancel, and type-mapping tests; `git diff --check`; and `npm run fallow:changed`.
  - Caveat: this is a safe helper cleanup, not a full task-detail redesign. `getTaskDetail` remains a large hotspot and any further split should target timeline/transcript/planned-execution boundaries with direct tests.
