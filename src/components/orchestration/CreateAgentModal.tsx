...
interface Props {
  ...
  companySlug: string;
  companyCode: string;
  onCreated?: (agent?: OrchestrationAgent) => void;
}
...
export function CreateAgentModal({ open, onClose, companySlug, companyCode, onCreated }: Props) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const json = (await res.json().catch(() => ({}))) as {
        error?: string;
        agent?: Partial<OrchestrationAgent>;
      };
      if (!res.ok) throw new Error(json.error || "Failed to create agent");
      const createdAgent = normalizeCreatedAgent(json.agent, {
        name: name.trim(),
        role: role.trim() || "General",
        model,
        adapterType: runtime?.provider ?? undefined,
        runtimeSlug: runtime?.runtimeSlug ?? undefined,
      });
      if (createdAgent) {
        window.dispatchEvent(new CustomEvent("orchestration-company-refresh", {
          detail: { companySlug, agent: createdAgent },
        }));
      }
      reset();
      onCreated?.(createdAgent ?? undefined);
      onClose();
    } catch (err) {
      ...
      <div role="dialog" aria-modal="true" aria-label="Create new agent" style={{ width: "100%", maxWidth: "560px", borderRadius: "16px", border: "1px solid var(--border)", background: "var(--modal-glass)", boxShadow: "var(--shadow-glass)", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderTop: "none", borderBottom: "none" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "3px 8px", borderRadius: "6px", border: "1px solid var(--border)", background: "var(--surface)", fontSize: "11px", fontWeight: 700, letterSpacing: "0.08em", color: "var(--text-secondary)" }}>{companyCode}</span>
            <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>›</span>
            <span style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-primary)" }}>New agent</span>
          </div>
          <button type="button" onClick={handleClose} style={{ width: "28px", height: "28px", borderRadius: "8px", border: "1px solid var(--border)", background: "transparent", color: "var(--text-muted)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><X size={14} /></button>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", padding: "8px 20px 18px", borderTop: "none", borderBottom: "none" }}>
          <button type="button" onClick={handleClose} disabled={saving} style={{ padding: "8px 18px", borderRadius: "10px", border: "1px solid var(--border)", background: "transparent", fontSize: "13px", fontWeight: 600, color: "var(--text-secondary)", cursor: saving ? "default" : "pointer", opacity: saving ? 0.6 : 1 }}>Cancel</button>
          <button type="button" onClick={() => void onSubmit()} disabled={saving || !canSubmit} style={{ padding: "8px 20px", borderRadius: "10px", border: "1px solid var(--border)", background: canSubmit ? "color-mix(in srgb, var(--text-primary) 72%, var(--surface))" : "var(--surface-hover)", fontSize: "13px", fontWeight: 600, color: canSubmit ? "var(--surface)" : "var(--text-muted)", cursor: canSubmit ? "pointer" : "default", transition: "all 120ms ease", opacity: saving ? 0.6 : 1 }}>{saving ? "Creating..." : "Create agent"}</button>
        </div>
      </div>
    </div>
  );
...
function normalizeCreatedAgent(
  raw: Partial<OrchestrationAgent> | undefined,
  fallback: {
    name: string;
    role: string;
    model?: string;
  ...
  return {
    ...
    adapterType: raw.adapterType || fallback.adapterType,
    runtimeSlug: raw.runtimeSlug || fallback.runtimeSlug,
    openclawAgentId: raw.openclawAgentId,
    reportingTo: raw.reportingTo,
    reportingToName: raw.reportingToName,
    created: raw.created,
    updated: raw.updated,
  };
}
