...
const SERVICES = [
  { name: "hiverunner", backend: "systemd", label: "HiveRunner" },
];
...
function getLineColor(line: string): string {
  ...
  if (lower.includes("info") || lower.includes("[info]")) return "#f59e0b";
  if (lower.includes("success") || lower.includes("✓") || lower.includes("ready")) return "#4ade80";
  if (lower.startsWith("[stream]")) return "#f59e0b";
  return "#c9d1d9";
}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: "0" }}>
      ...
      <div style={{ padding: "1.5rem 1.5rem 1rem" }}>
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>
          Real-time log streaming for HiveRunner services.
        </p>
      </div>
          ...
          {SERVICES.map((svc) => (
            <button
              ...
              style={{
                ...
                border: "1px solid",
                cursor: "pointer",
                backgroundColor: selectedService.name === svc.name ? "var(--surface-hover)" : "var(--card-elevated)",
                color: selectedService.name === svc.name ? "var(--accent)" : "var(--text-secondary)",
                borderColor: selectedService.name === svc.name ? "var(--accent)" : "var(--border)",
              }}
            >
              {svc.label}
            </button>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: "0.5rem", alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: autoScroll ? "var(--surface-hover)" : "var(--card-elevated)",
              color: autoScroll ? "var(--positive)" : "var(--text-muted)",
              border: "1px solid", borderColor: autoScroll ? "var(--positive)" : "var(--border)",
              cursor: "pointer",
            }}
          >
            ↓ Auto
          ...
          <button
            ...
            style={{
              ...
              padding: "0.5rem 1rem",
              borderRadius: "0.5rem",
              backgroundColor: streaming ? "var(--surface-hover)" : "var(--surface-hover)",
              color: streaming ? "var(--negative)" : "var(--positive)",
              border: "1px solid",
              borderColor: streaming ? "var(--negative)" : "var(--positive)",
              cursor: "pointer",
              fontWeight: 600,
              fontSize: "0.875rem",
            }}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.375rem" }}>
          <Circle
            className="w-2 h-2"
            style={{ fill: streaming ? "#57d17b" : "#8b949e", color: streaming ? "#57d17b" : "#8b949e" }}
          />
          <span style={{ color: streaming ? "#57d17b" : "#8b949e" }}>
