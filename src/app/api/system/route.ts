...
import { HIVE_RUNNER_WORKSPACE, WORKSPACE_IDENTITY } from '@/lib/paths';

const WORKSPACE_PATH = HIVE_RUNNER_WORKSPACE;
...
function parseIdentityMd(): { name: string; creature: string; emoji: string } {
  try {
    ...
    return { name: 'HiveRunner Agent', creature: 'AI Agent', emoji: '🤖' };
  }
}
...
function getIntegrationStatus() {
  ...
  try {
    ...
    // Also count top-level botToken as 1 bot
    if (telegramAccounts === 0 && telegramConfig?.botToken) {
      telegramAccounts = 1;
    }
  } catch {}
  integrations.push({
