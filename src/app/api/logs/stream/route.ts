...
const ALLOWED_SERVICES = ['hiverunner', 'classvault', 'content-vault', 'postiz-simple', 'brain'];
const ALLOWED_BACKENDS = ['pm2', 'systemd'];

export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  const service = searchParams.get('service') || 'hiverunner';
