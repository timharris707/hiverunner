...
async function run() {
  ...
  await test("task launch label uses the assigned agent name when available", () => {
  ...
  await test("task-bound presenter copy foregrounds the assigned agent while keeping Voice Chat distinct", () => {
  ...
  await test("unbound presenter copy falls back to Voice Chat wording", () => {
