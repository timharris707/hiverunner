...
async function run() {
  ...
  await test("synthesizes both status and comment actions from one direct task request", () => {
  ...
  await test("fills in only the missing action when Voice Assistant already proposed one tool request", () => {
    const intents = synthesizeDirectTaskActionIntents(
  ...
  await test("synthesizes start_task_work when the operator asks the bound agent to pick up work", () => {
    const intents = synthesizeDirectTaskActionIntents(
  ...
  await test("returns no synthetic intents for non-action chatter", () => {
    const intents = synthesizeDirectTaskActionIntents(
