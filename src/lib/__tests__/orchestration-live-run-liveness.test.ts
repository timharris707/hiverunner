...
async function run() {
  ...
  await test("running run with a fresh event is live", () => {
  ...
  await test("probeRunnerPidAlive detects the current process as alive", () => {
    assert.strictEqual(probeRunnerPidAlive(process.pid), true);
  });

  await test("probeRunnerPidAlive returns false for a PID that does not exist", () => {
