...
function heartbeatActionLedgerPayload(input: HeartbeatActionLedgerInput) {
  ...
  return {
    ...
    rawJson: input.entry.block.rawJson,
    rawStartOffset: input.entry.block.rawStartOffset,
    rawEndOffset: input.entry.block.rawEndOffset,
  ...
  for (const [messageIndex, text] of assistantTexts.entries()) {
    ...
    for (const block of blocks) {
      recordRuntimeActionLedgerEntry(input.db, {
        ...
        rawStartOffset: block.rawStartOffset,
        rawEndOffset: block.rawEndOffset,
