...
export function buildHeartbeatPrompt(
  ...
  sections.push("- Comment on a task: `add_comment` with `{ \"taskKey\": \"TASK-XXX\", \"body\": \"...\" }`. Lead supervisor ticks must also include `\"source\": \"lead-supervisor\"`.");
  ...
  sections.push("- Capture browser proof: `capture_browser_proof` with `{ \"taskKey\", \"baseUrl\", \"specs\": [\"e2e/example.spec.ts\"], \"urls\": [{ \"path\": \"/ACME/tasks\", \"label\": \"tasks-board\" }] }`. Use this before spending model time on screenshot or visual-QA tasks. HiveRunner runs Playwright, saves screenshots/videos, writes a manifest with hashes, registers it as the task artifact, and posts a concise proof comment.");
  ...
  sections.push("- Propose goal work for operator review: `propose_sprint_plan` should use Plan Mode when you are lead for a company goal: `{ \"companyGoalId\", \"planMode\": true, \"sprints\": [{ \"sequenceNumber\", \"name\", \"objective\", \"successCriteria\", \"validationChecks\", \"outOfScope\", \"defaultExecutionEngine\", \"defaultModelLane\", \"tasks\": [{ \"title\", \"description\", \"assignee\", \"priority\", \"type\", \"executionEngine\", \"modelLane\", \"dependsOn\", \"validation\" }] }] }`. This creates drafts only; do not create execution tasks directly when planning a goal. Task \"priority\" is optional and must be one of P0|P1|P2|P3 (P0 = most urgent; common words like high/medium/low are coerced; unrecognized values fall back to the default). Task \"type\" is optional, one of: feature|bug|maintenance|research|infrastructure|directive|epic|spike|docs|infra|refactor|review|qa|release. The old single-sprint `{ \"sprint\", \"tasks\" }` shape still works only for explicitly one-sprint work.");
  ...
  sections.push("Final-answer example:\n```mc-action\n{\"action\":\"add_comment\",\"taskKey\":\"TASK-XXX\",\"body\":\"**Summary**\\n\\nCompleted the research and included the three strongest sources with links.\"}\n```\n```mc-action\n{\"action\":\"update_task\",\"taskKey\":\"TASK-XXX\",\"status\":\"done\"}\n```");
