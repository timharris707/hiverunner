...
export function buildHeartbeatPrompt(
  ...
  sections.push("- Capture browser proof: `capture_browser_proof` with `{ \"taskKey\", \"baseUrl\", \"specs\": [\"e2e/example.spec.ts\"], \"urls\": [{ \"path\": \"/INS/tasks\", \"label\": \"tasks-board\" }] }`. Use this before spending model time on screenshot or visual-QA tasks. HiveRunner runs Playwright, saves screenshots/videos, writes a manifest with hashes, registers it as the task artifact, and posts a concise proof comment.");
