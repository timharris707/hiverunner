...
const KNOWN_FIELDS = new Set(["status", "owner", "date", "tags", "summary"]);
const SUMMARY_LIMIT = 180;

export function parseDecisionLogMarkdown(markdown: string): DecisionLogSummary {
  const normalized = markdown.replace(/\r\n?/g, "\n").trimStart();
  const { metadata, body, warnings } = parseMetadata(normalized);
  const title = extractTitle(body) ?? metadata.title ?? "Untitled decision";
  const summarySource = metadata.summary ?? extractFirstParagraph(body) ?? "";

  for (const key of Object.keys(metadata)) {
    if (!KNOWN_FIELDS.has(key) && key !== "title") {
  ...
  return {
    ...
    summary: shortenSummary(summarySource),
    warnings,
  };
}
...
export function summarizeDecisionLogFiles(filePaths: string[]): string {
  return filePaths
    .map((filePath) => {
      const markdown = readFileSync(filePath, "utf8");
      return formatDecisionLogSummary(parseDecisionLogMarkdown(markdown));
    })
    .join("\n\n");
}
...
if (require.main === module) {
  ...
  console.log(summarizeDecisionLogFiles(files));
}
